...
> **중요**: 2025년 4월부터 Clerk의 네이티브 Supabase 통합을 사용합니다. JWT Template은 더 이상 필요하지 않습니다 (deprecated).

**3-1. Clerk Dashboard에서 Supabase 통합 활성화**

1. Clerk Dashboard → **Setup** → **Supabase Integration**
2. **"Activate Supabase integration"** 클릭
3. 표시되는 **Clerk Domain** 복사 (예: `your-app-12.clerk.accounts.dev`)
...
7. **"Create connection"** 클릭

**3-3. 통합 확인**

- [Clerk 공식 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)에서 상세 정보 확인
- [프로젝트 통합 가이드](./docs/CLERK_SUPABASE_INTEGRATION.md) 참고
...
모든 Clerk 컴포넌트(로그인, 회원가입, 사용자 프로필 등)가 한국어로 표시됩니다.
...
Supabase의 기본 Auth를 사용하는 방식입니다. 공개 데이터나 인증이 필요 없는 데이터에 적합합니다.
...
자세한 내용은 [Supabase 설정 가이드](./docs/SUPABASE_SETUP.md)를 참고하세요.
...
│   ├── integration-test/ # Clerk + Supabase 통합 테스트
│   ├── instruments/       # Supabase 공식 방식 예시 페이지
...
│   │   ├── server-browser.ts  # Supabase 공식 방식 (Server)
│   │   ├── browser.ts          # Supabase 공식 방식 (Client)
│   │   ├── clerk-client.ts    # Clerk 통합 (Client)
│   │   ├── server.ts          # Clerk 통합 (Server)
│   │   ├── service-role.ts    # 관리자용
│   │   └── client.ts          # 레거시 (deprecated)
...
### 프로젝트 문서
- [Clerk + Supabase 통합 가이드](./docs/CLERK_SUPABASE_INTEGRATION.md) - Clerk와 Supabase 네이티브 통합 방법
- [Supabase 설정 가이드](./docs/SUPABASE_SETUP.md) - Supabase 공식 방식 설정 및 사용법
- [Clerk 한국어 로컬라이제이션 가이드](./docs/CLERK_LOCALIZATION.md) - Clerk 컴포넌트 한국어 설정 방법
